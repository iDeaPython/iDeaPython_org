...
class ship():
    def __init__(self, Host, test=0, Ldata_len=26, initSlot=1):
        ap = network.WLAN(network.AP_IF)  # create access-point interface
        ap.active(1)  # activate the interface
        ap.config( essid=Host.ssid, password=Host.passwd, authmode=4, channel=9 )  # set the ESSID of the access point
        time.sleep_ms(3000)
        self.test = test  # only for test, control by main.py
        self.shake_port = 700
    ...
    def getManageName(self,name):
        return getattr( __import__( 'lib.COMMON.management', globals(), locals(), [name, ], 0 ), name )
    def updateSwitch(self, apparatus, CMD):
        self.switch_dict['%04d' % apparatus.ID] = [CMD.strip(), True]
        print('%04d' % apparatus.ID, 'update switchDict: ', self.switch_dict['%04d' % apparatus.ID])

    def assistFire(self,name,cmd):
        try:
            globals()[name]
        except:
            # globals()[name] = getattr(__import__('lib.COMMON.management', globals(), locals(), [name,], 0),name)
            globals()[name] = self.getManageName(name)
        self.updateSwitch(apparatus=globals()[name], CMD=cmd)
        ...
        if self.RunAfter('newConn_1100'):
            try:
                ...
                try:
                    ...
                    print('no shakeData, it may be a GPIO') # what?
                    pass

                if shakeData[:4] == '0000':
                    ID=self.assignID()
                    print('send generate ID: ', ID)
                    self._send_data(conn1, ID + '__')
                    conn1.settimeout(0)
                    conn1.close()
                    return 0
                conn1.settimeout(0)
    ...
    def assignID(self):
        for i in range(len(self.storeID)+1):
            i += 1
            try:
                self.storeID.__getitem__('%04d' % i)
            except:
                self.storeID['%04d' % i] = i
    ...
    def _get_L(self):
        recv = self._recv_data(self.N_socket, self.shakeData_len + self.Ldata_len)  # recv has included accept()
        if recv != '_':
            shakedata = recv[:self.shakeData_len]
            uniqueid = shakedata[:4]
            rawdata = recv[self.shakeData_len:]
            self.onlineLS_ID[uniqueid] = 1
            print ('get L',self.onlineLS_ID)

            if self.test:
                print( 'received LS raw data: ' + rawdata )

            if shakedata[-2:] == 'S_':  # 'receive a swicth request'
                try: # check the id in switch_dict
                    a= self.switch_dict[uniqueid][1]
                    if  self.test:
                        print('ID ' + uniqueid + ' can be updated: ', a)
                except:
                    if  self.test:
                        print('ID ' + uniqueid + ' ask to update, but not in switch_dict so far')
                    a = 0
                if a:
                    self._send_data(self.connAccept, shakedata + self.switch_dict[uniqueid][0])
                    self.switch_dict[uniqueid][1] = False
                    print('\nsending updated data to client :', shakedata + self.switch_dict[uniqueid][0],
                          '\n\n')
                elif rawdata == 'check':
                ...
                # for ASSIST
                if 'assist' in rawdata: # for assist fire
                    str1=rawdata[6:]
                    nameAlter=str1[:str1.find('!')]
                    cmd=str1[str1.find('!')+1:]
                    self.assistFire(nameAlter,cmd)
                    if self.test:
                        print( 'received assist fire data' )
                # for remote Link
                elif rawdata[:3]=='Btn' and 'Slot' in (rawdata):
                    data,name=rawdata.split('!')
                    print ('the name need to be altered is :',name)
                    self.linkAlterList['%04d' % self.getManageName(name).ID]=data
                    if self.test:
                        print( 'received change link data ' )

                self.connAccept.close()

            elif uniqueid in self.linkAlterList:
                self._send_data( self.connAccept, shakedata + self.linkAlterList[uniqueid] )
    ...
    def Main(self):  # put it into "while" statement
        ...
        self.wait_newconn()
        self.start_server()
        if Timer.RunAfter('zzzxxx_30000'):
            if self.test:
                print('sentry patrols online')
                print('get L', self.onlineLS_ID)
            for k in self.onlineLS_ID.keys():
                list1.append(k)
            for k in list1:
                if self.onlineLS_ID[k] == 0:
                    self.onlineLS_ID.pop(k) # delete offline ID
                    self.storeID.pop(k) # delete offline ID
                    print ('collector offline, deleted ID:', k)
