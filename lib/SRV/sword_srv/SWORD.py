...
class Sword(): # using Timer
    ...
    def runWebrepl(self, v, w, x, y):
        ...
        if self.webreplON:
            ...
            if v >40:
                ...
                print ('ready for webrepl :',v) # for test.bat
                return v
    ...
    def activate_car(self, GYRO):
        ...
        if self.TH1 and self.spinCount>45:
            self.conn.updateSwitch(apparatus=self.Lhand, CMD=Sword_AP.CMD.Lhand)
            self.conn.updateSwitch(apparatus=self.Rhand, CMD=Sword_AP.CMD.Rhand2)
            self.TH1 = 0
        elif self.TH1==0 and self.RunAfter('act_car',3000):
            print ('\nactivate_car\n')
            self.clickRelay(1.2)
            # self.TH1 = 1
            return 1
    ...
    def activate_target(self, GYRO):
        ...
        # if self.L3 or 1: # for test.bat
        if self.runTarget : # officially
            # if self.doneTARGET==0 and self.RunAfter( ''ii_3000'' ):
            if self.doneTARGET==0 and self.RunAfter( 'acv_target',3000 ):
                print ('run target')
                self.conn.updateSwitch(apparatus=self.target, CMD=Sword_AP.CMD.target) # activate target
                # self.conn.updateSwitch(ID=self.LhandID, cmd=Sword_AP.CMD.Lhand)  # for test.bat, because i need a hint
                print(self.conn.switch_dict, '\n')
                self.doneTARGET=1
            elif self.doneTARGET and self.RunAfter('done_target',10000):
        ...
        # elif self.FP and self.BP and self.LP and self.RP or 1:  # for test.bat
        elif self.FP and self.BP and self.LP and self.RP:  # officially
            self.FP, self.BP, self.LP, self.RP = 0, 0, 0, 0
            self.conn.updateSwitch(apparatus=self.Rhand, CMD=Sword_AP.CMD.Rhand2)  # the hand which hold the sword_srv, just activate 3s
            # self.conn.updateSwitch(ID=self.LhandID, cmd=Sword_AP.CMD.Lhand) # for test.bat, because i need to see the both when im testing
            self.runTarget = 1

        # elif GO == 'F' and j > 7 or 1: # for test.bat
            ...
            if self.FP == 0:
                ...
                self.FP = 1
        # elif GO == 'B' and j > 7 or 1 : # for test.bat
            ...
            if self.BP == 0:
                ...
                self.BP = 1
        # elif l == 'L' and i > 7 or 1:# for test.bat
            ...
            if self.LP == 0:
                ...
                self.LP = 1
        # elif l == 'R' and i > 7 or 1:# for test.bat
    ...
    def run(self,GYRO):
        ...
        if self.runON: # for officially, why do i need runON, when go for webrepl, self.runON=0
            if GYRO==None: # when no install GYRO, test.bat program
                GYRO='L00-F00-S00'
            if self.LaserON==0:
                self.clickRelay(0.2)
